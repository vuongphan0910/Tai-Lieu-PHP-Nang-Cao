...
<form id="form1" name="form1" method="get" action="">
  <label for="tukhoa">Nhap tu khoa:</label>
  <input type="text" name="tukhoa" id="tukhoa" />
  <input type="submit" name="tim" id="tim" value="Tim kiem" />
</form>
<p>&nbsp;</p>
<?php
...
<table width="560" border="1">
  ...
  $kq=mysql_query("select * from webtm_sanpham where TenSP like '%$tukhoa%'");
  ...
  //Lay du lieu:
  $sl="select * from webtm_sanpham where TenSP like '%$tukhoa%' limit $vitri,$sd";
  $kqsp=mysql_query($sl);
...
for($i=1;$i<=$tst;$i++){
	...
	?><a href="phantrang_tim(2).php?page=<?php echo $i;?>&tukhoa=<?php echo $tukhoa;?>"><?php echo $i;?></a> &nbsp;<?php 
	} //ket thuc else
} // ket thuc for
